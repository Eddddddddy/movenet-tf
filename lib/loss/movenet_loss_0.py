...
class MovenetLoss(Loss):
    ...
    def call(self, y_pred, y_true):  # y_true: target and kps_mask, y_pred: output
        ...
        heatmaps = y_true[:, :, :, :17]
        # kps_mask = y_true[0, 0, 86:]

        heatmap_loss = self.heatmapLoss(y_pred, heatmaps, 128)

            # bg_loss = self.bgLoss(output[0], heatmaps)
            # bone_loss = self.boneloss(output[0], heatmaps)
        bone_loss = self.boneLoss(y_pred, heatmaps)
            # print(heatmap_loss)
            ...
            # return all_loss
            # loss.append(heatmap_loss+bone_loss)
        return heatmap_loss + bone_loss
