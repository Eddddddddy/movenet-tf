...
######## dataloader
def TensorDataset_tf(data_labels, img_dir, img_size, dataset_h5, data_aug):
    interp_methods = [2, 3, 0, 1]

    # def __getitem__(self, index):

    for item in data_labels:
        # item = data_labels[index]
        ...
        img = dataset_h5[item["img_name"]]

        #### Data Augmentation
        # print(item)
        if data_aug is not None:
            img, item = data_aug(img, item)
        # print(item)
        # print()
        ...
        heatmaps, sigma = label2heatmap(keypoints, other_keypoints, img_size)  # (17, 48, 48)
        # 超出边界则设为全0
        ...
        cx = min(max(0, int(center[0] * img_size // 4)), img_size // 4 - 1)
        cy = min(max(0, int(center[1] * img_size // 4)), img_size // 4 - 1)
        # if '000000103797_0' in item["img_name"]:
        #     print("---data_tools 404 cx,cy: ",cx,cy, center)

        centers = label2center(cx, cy, other_centers, img_size, sigma)  # (1, 48, 48)
        ...
        regs = label2reg(keypoints, cx, cy, img_size)  # (14, 48, 48)
        ...
        offsets = label2offset(keypoints, cx, cy, regs, img_size)  # (14, 48, 48)
        # for i in range(14):
        #     print(regs[i,y,x])
